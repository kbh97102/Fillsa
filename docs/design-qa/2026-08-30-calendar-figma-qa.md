...
|---|---|---|---|
| Calendar header | top_mobile descendants | logo Home action, real streak, profile action | Build/test passed; live empty capture exists |
| Calendar shell | `2985:21954` | Sunday-first fixed 320 × 396 month/grid/selected date | RED→GREEN order test; live empty capture exists |
...
### Round 3 — follow-up P1 review

| Scope | Difference | Fix | Result |
|---|---|---|---|
| Weekday header | Kotlin `DayOfWeek.entries` defaulted to Monday-first while the grid is Sunday-first | Added and consumed `calendarWeekdayOrder()` with literal Sunday→Saturday order | RED→GREEN focused test |
| Basic empty companion | 80dp quote card could be visually lost behind the preserved shared bottom surface | Made Empty detail content explicitly require the 80dp quote card and retained its Figma overlap placement; corrected 360 × 816 capture places it from y600 to y680, immediately above the shared bar | RED→GREEN presentation test; runtime capture |
| Header action | Calendar logo had lost its established Home navigation | Restored `Screens.Home()` click action; profile still routes to My page | Build/test |
| Evidence durability | Figma/runtime evidence was only under `/private/tmp` | Committed basic, selected/completed, expanded/completed Figma PNGs and the 360 × 816 runtime PNG under the QA asset directory | Complete |

- Focused RED: `./gradlew :presentation:testDebugUnitTest --tests com.arakene.presentation.ui.calendar.CalendarRecordIndicatorTest` failed because `calendarWeekdayOrder` and `calendarDetailContent` were unresolved.
- Focused GREEN: the same test task passed after the real order/detail-content helpers were added and consumed.
...
- Controlled reimplementation basic capture: `docs/design-qa/assets/calendar-figma-2985-21952/rework-8464dbf/runtime-basic-light-emulator-360x816.png`, Android Emulator `emulator-5556`, temporary 360 × 816 / 160dpi and the existing app Light theme. It includes status/safe/nav/ad surfaces and was taken from the live empty selection. The 80dp quote card is fully laid out from y600 through y680 above the app bar; the Android emulator's external `AdMob native ad validator` overlay can visually cover that lower region but is not an app surface.
