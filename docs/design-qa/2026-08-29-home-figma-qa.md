...
| Prompt response | Figma `2929:13630` specifies question, answer area, counter, and record CTA. Home has no question feed, while existing Typing `korTyping`/`engTyping` are quote-transcription state and cannot represent a prompt answer. | Extracted `HomePromptAnswerSection`: preserved the established static Figma question placeholder, uses 17dp/`#DED4BD`/50% white answer box, 200-grapheme input limit/count, accessible input/CTA labels, and the exact previously downloaded 18dp CTA SVG. The CTA now sends the canonical draft through `HomeAction.ClickQuote` → `Screens.DailyQuote.initialAnswer`; Typing initializes an editable, accessible answer-draft field from that argument without corrupting quote-transcription state. | `HomeAnswerInputStateTest` and `HomeAnswerRouteTest` RED→GREEN verify the grapheme cap, route payload, Typing draft initialization, and empty-route compatibility. Dynamic question data remains Blocked; the answer draft has no invented backend persistence. Runtime visual comparison pending. |
